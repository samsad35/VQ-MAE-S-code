...
from ...model import SpeechVQVAE, MAE
...
class MAE_Train(Train):
    def __init__(self, mae: MAE, vqvae: SpeechVQVAE,
        ...
        """ Follow """
        if follow:
            self.follow = Follow("rsmae", dir_save=r"checkpoint", multigpu_bool=multigpu_bool, description=description)
    ...
    @staticmethod
    def inverse_tuple(input, size_patch=4, depth_t=5):
        t1 = input.shape[1] // 16
        input = rearrange(input, 'b (t1 c1) (l1 t2) -> b (t1 t2) (c1 l1)', t1=t1, t2=depth_t, c1=16, l1=size_patch)
        return input
    ...
    def one_epoch(self):
        ...
        for img in tqdm(iter(self.training_loader)):
            ...
            if self.tube_bool:
                indices = self.to_tube(indices, size_patch=4, depth_t=10)
    ...
    def eval(self):
        ...
        if self.tube_bool:
            indices = self.inverse_tuple(indices, size_patch=4, depth_t=10)
            predicted_indices = self.inverse_tuple(predicted_indices, size_patch=4, depth_t=10)
            images_mask = self.inverse_tuple(images_mask, size_patch=4, depth_t=10)
        self.save_wav(indices, save=f"{self.follow.path_samples}/original.wav", vqvae=self.vqvae)
        self.save_wav(predicted_indices, save=f"{self.follow.path_samples}/reconstructed.wav", vqvae=self.vqvae)
        self.save_wav(images_mask, save=f"{self.follow.path_samples}/masked.wav", vqvae=self.vqvae)
        return losses
