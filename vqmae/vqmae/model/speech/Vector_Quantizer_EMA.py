...
class VectorQuantizerEMA(nn.Module):
    ...
    def get_code_indices(self, input):
        ...
        inputs = input.permute(0, 2, 1).contiguous()
        input_shape = inputs.shape
        ...
        # Encoding
        encoding_indices = torch.argmin(distances, dim=1).unsqueeze(1)
        return encoding_indices, input_shape, inputs.device

    def vq(self, encoding_indices, input_shape, device):
    ...
    def quantify(self, encoding_indices):
        image_embeds = self._embedding(encoding_indices)
        image_embeds = rearrange(image_embeds, 'b n d -> b d n')
        return image_embeds
